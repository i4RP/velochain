...
fn test_txpool_add_and_retrieve() {
    ...
    let tx = Transaction::new_game_action(27181, 0, GameAction::Move { x: 100, y: 200, z: 0 });
...
fn test_game_world_tick() {
    ...
    let root2 = game_world.tick(&[(
        "0x1234567890abcdef1234567890abcdef12345678".to_string(),
        GameAction::Move { x: 1000, y: 2000, z: 0 },
...
fn test_game_world_player_spawn() {
    ...
    game_world.tick(&[(
        ...
        GameAction::Move { x: 5000, y: 10000, z: 0 },
    )]).expect("tick");
...
fn test_world_state_balance_ops() {
    ...
    // Add balance
    state.add_balance(&addr, U256::from(1000)).expect("add balance");
    let balance = state.get_balance(&addr).expect("get balance");
    assert_eq!(balance, U256::from(1000));

    // Add more balance
    state.add_balance(&addr, U256::from(500)).expect("add balance");
    let balance = state.get_balance(&addr).expect("get balance");
    assert_eq!(balance, U256::from(1500));

    // Sub balance
    state.sub_balance(&addr, U256::from(200)).expect("sub balance");
    let balance = state.get_balance(&addr).expect("get balance");
...
fn test_websocket_event_channel() {
    ...
    match event {
        GameEvent::NewBlock { number, tx_count, .. } => {
            assert_eq!(number, 1);
...
fn test_websocket_game_tick_event() {
    ...
    let event = rx.try_recv().expect("recv event");
    match event {
        GameEvent::GameTick { tick, entity_count, player_count, .. } => {
            assert_eq!(tick, 42);
